...
@customElement('spinner-card-editor')
export class SpinnerCardEditor extends LitElement implements LovelaceCardEditor {
  @property({ attribute: false }) public hass?: HomeAssistant;
  @internalProperty() private _config?: SpinnerCardConfig;
  ...
  get _force_native_time_picker_for_device(): boolean {
    return localStorage.getItem('spinnerCard.forceNativePicker') === 'true';
  }
  ...
  private _valueChanged(ev): void {
    ...
    if (target.configValue) {
      if (target.configValue === 'force_native_time_picker_for_device') {
        localStorage.setItem('spinnerCard.forceNativePicker', target.checked ? 'true' : 'false');
...
wdw.customCards.push({
  ...
  name: 'Spinner Card',
  preview: false,
  description: 'Coordinate spinner settings in a beautiful way',
});
