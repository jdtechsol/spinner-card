...
import { SpinnerCardConfig } from './types';
...
export class SpinnerCard extends LitElement {
  ...
  @internalProperty() private config?: SpinnerCardConfig;

  public setConfig(config: SpinnerCardConfig): void {
  ...
  private _showTimeInputs(): void {
    const timeInputWrap = this.shadowRoot!.getElementById('spinner-wrapper');
  ...
  private _selectedTimeValueChangedPicker(ev): void {
    ...
    if (localStorage.getItem('spinnerCard.forceNativePicker') === 'true') {
      ...
      const hourInput = this.shadowRoot!.getElementById('spinner-time-input-hour') as HTMLInputElement;
      hourInput.value = newTime.split(':')[0];
      hourInput?.blur();
      clearTimeout(this.inputBlurTimer);
      const minuteInput = this.shadowRoot!.getElementById('spinner-time-input-minute') as HTMLInputElement;
  ...
  private _saveTimePickerValue(newTime: string): void {
    ...
    const leftPicker = this.shadowRoot!.getElementById('spinner-time-picker-left') as HTMLInputElement;
    leftPicker.value = newTime;
    const rightPicker = this.shadowRoot!.getElementById('spinner-time-picker-right') as HTMLInputElement;
  ...
  private _saveTimeInputValue(): void {
    const stateObj = this.hass!.states[this.config!.time_entity];
    const timeInputHour = this.shadowRoot!.getElementById('spinner-time-input-hour') as HTMLInputElement;
    const timeInputMinute = this.shadowRoot!.getElementById('spinner-time-input-minute') as HTMLInputElement;
    ...
    if (
      ...
      console.error(
        `Values for spinner time of '${timeInputHour.value}' hours and '${timeInputMinute.value}' is not a valid time!`,
      );
      return;
    }
  ...
  private _timePickerClick(focusHours: boolean): void {
    if (localStorage.getItem('spinnerCard.forceNativePicker') === 'true') {
      ...
      if (focusHours) {
        const input = this.shadowRoot!.getElementById('spinner-time-input-hour') as HTMLInputElement;
        input?.focus();
        this._moveCursorToEnd(input);
      } else {
        const input = this.shadowRoot!.getElementById('spinner-time-input-minute') as HTMLInputElement;
        input?.focus();
        this._moveCursorToEnd(input);
      }
    }
